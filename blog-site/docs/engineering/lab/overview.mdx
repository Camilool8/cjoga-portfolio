...
sidebar_position: 1
title: Lab overview
image: img/og/engineering-lab-overview.png
...
<Timeline caption="Three iterations">
  <TimelineEntry
    year="v1"
    title="Three nodes on WiFi · mixed-distro · flannel cascade on every dropout"
  />
  <TimelineEntry
    year="v2"
    title="Wired core · Tailscale mesh for everything off-LAN"
  />
  <TimelineEntry
    year="v3"
    title="Proxmox VM control plane · Envoy Gateway · Cloudflare Tunnel"
    emphasis
  />
</Timeline>
...
The bigger problem was the WiFi itself. When a node briefly dropped its connection, or when it hit disk or memory pressure, [flannel](https://github.com/flannel-io/flannel) on that node would fall over — and the failure rippled, taking pod-to-pod networking down across the rest of the cluster. Manual recovery became the daily mode.
